        ...
        <div className="drawer-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDrawerOpen(false) }}>
          <aside className="expense-drawer" role="dialog" aria-modal="true" aria-labelledby="drawer-title">
            ...
            <form onSubmit={(event) => void saveExpense(event)}>
              ...
              <div className="amount-grid"><label>Planned amount<div className="money-input"><span>$</span><input type="number" min="0" step="0.01" placeholder="0.000" value={form.planned} onChange={(event) => setForm({ ...form, planned: event.target.value })} /></div></label><label>Actual amount<div className="money-input"><span>$</span><input type="number" min="0" step="0.01" placeholder="0.00" value={form.actual} onChange={(event) => setForm({ ...form, actual: event.target.value })} /></div></label></div>
        ...
        <div className="drawer-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSalaryDrawerOpen(false) }}>
          <aside className="expense-drawer" role="dialog" aria-modal="true" aria-labelledby="salary-drawer-title">
            ...
            <form onSubmit={(event) => void saveSalary(event)}>
              ...
              <label>Base salary<div className="money-input"><span>$</span><input type="number" min="0" step="0.001" placeholder="0.00" autoFocus value={salaryForm.baseSalary} onChange={(event) => setSalaryForm({ ...salaryForm, baseSalary: event.target.value })} /></div></label>
              {error && <div className="form-error">{error}</div>}
